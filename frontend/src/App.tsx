...
export default function App() {
  ...
  const deletePastAnalysis = async (rec: any) => {
    if (!window.confirm(`Are you sure you want to delete "${rec.title || 'this analysis'}"?`)) return;
    try {
      await api(`/api/analyses/${rec.id}`, { method: 'DELETE' });
      setSavedAnalyses(prev => prev.filter(x => x.id !== rec.id));
      try {
        const local = JSON.parse(localStorage.getItem('saved_analyses') || '[]');
        const updated = local.filter((x: any) => x.id !== rec.id);
        localStorage.setItem('saved_analyses', JSON.stringify(updated));
      } catch {}
      setToast('Analysis deleted successfully');
    } catch {
      try {
        const local = JSON.parse(localStorage.getItem('saved_analyses') || '[]');
        const updated = local.filter((x: any) => x.id !== rec.id);
        localStorage.setItem('saved_analyses', JSON.stringify(updated));
        setSavedAnalyses(prev => prev.filter(x => x.id !== rec.id));
        setToast('Analysis deleted locally');
      } catch {
        setToast('Failed to delete analysis');
      }
    }
  };
        ...
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-emerald-200/90 font-medium">Ayurveda IP Assistant</div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 sm:p-8 max-w-[1500px] mx-auto">
          ...
          {page === 'dna' && (
            <DNA
              ...
              deletePastAnalysis={deletePastAnalysis}
              dnaSaved={dnaSaved}
            />
          )}
          {page === 'two' && (
      ...
      <div className="hidden lg:flex bg-[#082f25] text-white p-14 items-center">
        <div className="max-w-xl">
          ...
          <h1 className="serif text-5xl mt-4 leading-tight">
            Turn an Ayurveda innovation into a structured IP journey.
          </h1>
          <p className="text-emerald-100/70 mt-6 leading-7">
            Innovation DNA, Two-Sided AI, evidence-grounded analysis and practical next steps for Ayurveda formulations/innovations in one workspace.
          </p>
        </div>
      </div>
...
function DNA({ dna, setDna, save, go, savedAnalyses, loadPastAnalysis, deletePastAnalysis, dnaSaved }: any) {
              ...
              {savedAnalyses.map((rec: any) => (
                <div key={rec.id} className="p-4 border rounded-xl hover:border-emerald-700/40 transition flex justify-between items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => deletePastAnalysis(rec)}
